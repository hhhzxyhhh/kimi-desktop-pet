...
**同时开多个 Kimi Code 窗口也没问题**：每个会话单独跟踪，按优先级汇总——任何一个会话在等你批准或回答最优先，其次出错，再其次只要有会话在忙就上班，全部空闲才恢复默认；会话结束立即收点，终端被直接强杀（收不到 SessionEnd）时按进程活性检测立即清场，有活进程的长任务/等批准则一直保留（超时清场只兜底没有活进程的记录）。
