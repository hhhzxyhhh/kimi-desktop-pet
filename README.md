...
- **台前调度豁免**：panel 窗口类型 + macOS 原生 stationary 行为，开着台前调度也能在左侧保留区活动（仅 macOS）
...
**同时开多个 Kimi Code 窗口也没问题**：每个会话单独跟踪，按优先级汇总——任何一个会话在等你批准或回答最优先，其次出错，再其次只要有会话在忙就上班，全部空闲才恢复默认；**点 = 开着的窗口**：进程活着就一直留（不论忙闲），进程死了立即收点，超时清场只兜底没有活进程的记录。
多会话时它还会主动报告进度：**脚下一排指示点，数量 = 开着的会话窗口数**（绿点 = 在干活，灰点 = 空闲，红点 = 出错或在等你批准/回答，**点哪颗就 `kimi --session` 恢复那个会话；已开着的只唤前台不重复开**）；**哪个会话完成**，气泡直接点名（「项目名」搞定！）；右键菜单的"会话状态"里能看到每个会话在干嘛——行点开对应终端，悬停二级目录里的"取消监控"可以把某个会话藏起来（有新活动自动回来）。
...
| `display-areas.cjs` | 多显示器几何（纯 Node 模块，可单测） |
| `mac-window.cjs` | macOS 原生窗口行为（台前调度豁免/stationary Space，移植自 Clawd） |
...
| `test-display-areas.cjs` | 多显示器几何单元测试（纯 Node，跨平台） |
...
以下部分借鉴自 **[Clawd On Desk](https://github.com/rullerzhou-afk/clawd-on-desk)**（盯着 AI 编程 Agent 干活的桌面宠物，内置像素螃蟹 Clawd、三花猫 Calico 等主题）：

- macOS 台前调度左侧保留区豁免与全屏 Space 可见：`panel` 窗口类型 + NSWindow stationary 行为 + SkyLight 私有 Space（`mac-window.cjs` 移植自其 `src/mac-window.js`）
- Kimi Code 联动的状态归一化（normalizeToolName）与"新会话不继承残留状态"的思路

如果你用 Claude Code / Codex / Gemini / Cursor 里的任何一个，强烈推荐去养这只螃蟹；玩 Kimi Code 的话，这颗球也很乐意跟它做邻居。
