...
多会话时它还会主动报告进度：**脚下一排指示点，数量 = 开着的会话窗口数**（绿点 = 在干活，灰点 = 空闲，红点 = 出错或在等你批准/回答，**点哪颗就在终端里恢复那个会话**）；**哪个会话完成**，气泡直接点名（「项目名」搞定！）；右键菜单的"会话状态"里能看到每个会话在干嘛——行点开对应终端，悬停二级目录里的"取消监控"可以把某个会话藏起来（有新活动自动回来）。
